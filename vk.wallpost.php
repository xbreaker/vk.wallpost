...
################################################################
/** Функция для обхода варнинга CURLOPT_FOLLOWLOCATION cannot be activated when in safe_mode or an open_basedir is set in
...
private function auth() {
    if ($this->login=="") {$this->RaiseExeption("Empty login"); return false;}
...
private function getParams() {
    ...
    if ($this->wallId=="") $this->wallId=$f["my_id"];
    return $f;
  }
  
################################################################
/** Добавляет в альбом фотографию. Возвращает массив
...
################################################################
/** Зпрашивает страницу со стеной в поисках хеша. Если не находит, то пробует логиниться.
...
private function getHash(){
    ...
    if($h['my_id'] == 0) {$this->RaiseExeption("Can't log in"); return false;}
    return $h;       
}
...
public function postPicture($imgUrl, $linkTo='') {
      ...
      pr($img);
      if (!$img) {$this->RaiseExeption("Picture is not uploaded");return false;}
